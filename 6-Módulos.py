...
Armazenando sua funções em módulos
    ...
    Quando armazenamos funções em arquivos separados, podemos compartilhar esses arquivos com outros programadores sem a ncessidade de compartilhar o programa todo. Saber como importar funções também possibilita usar bibliotecas de funções que outros programadores escreveram.
...
Importando um módulo completo
    ...
    Agora iremos importar para cá, quando Python lê esse arquivo, a linha import pizza lhe diz para abrir o arquivo pizza.pye copiar todas as funções dele para esse programa e agora qualquer função de pizza.py estará disponível aqui.
    Para chamar a função que está em um módulo importado, forneça o nome do mpodulo e em seguida o nome da função separados por um ponto.
...
Usando a palavra reservada #as para atribuir um alias a uma função
    Se o nome de um função que você importar poder entrar em conflito com um nome já existente em seu programa, ou se o nome da função for longo, podemos usar um alias único, que é um nome alternativo, semelhante a um apelido para a função. Observe o exemplo abaixo que foi dado um apelido para a função e em seguida a chamada foi realizada através dele mesmo.
...
"""
Usando a palavra reservada #as para atribuir uma alias a um módulo
    Também podemos fornecer um alias para um nome de módulo, isso permite chamar mais rapidamente as funções do módulo. Observe o exemplo abaixo, e suponha que tivéssemos a chamda através de #p, seria muito mais compacto.Utilizar essa sintaxe tambpem  permite dar enfoque aos nomes descritivos de suas funções, nomes esse que devem claramente informar o que cada função faz, algo mais importante para a legibilidade de seu código que usar o nome completo do módulo.
...
Importando todas as funções de um módulo
    ...
    A melhor aboradgem é importar a função que você quiser ou importar o módullo todo e usar a notação do ponto. Isso resulta em um código claro, mais fácil de ler e de entender.
...
Estilizando funções
    ...
    Um módulo nõa deve começar seu nome com um número, isso irá gerar um erro de sintaxe.
