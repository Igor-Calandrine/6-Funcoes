...
"""Modificando uma lista em uma função
    ...
    Em #u definomos a função com dois parâmetros, uma lista a serem impressos e outra a serem concluídas. Dada as duas listas, a função simula os esvaziamento de uma lista e em seguida preenchendo outra.
    Em #v definimos a função com um parãmetro para apresentar a lista de modelos finalizados com o nome de cada modelo.
    O código produz a mesma saída que poderia produzir com apenas uma função, mas observe no corpor de #x como o código fica muito mais fácil de entender."""
...
def print_model(unprited_models, printed_models): #u
...
def show_printed_model(printed_models): #v
    print("\nOs seguintes modelos foram finalizados")
    for printed_model in printed_models:
        print(f"\t{printed_model.title()}")

print_model(unprited_models, printed_models) #x
show_printed_model(printed_models) #x
...
"""
    ...
    Esse exemplo também mostra a ideia de que toda dunção deve ter uma tarefa específica, isso é mais vatajoso que isar um única função para executar as duas tarefas. Se você estiver escrevendo uma função e perceber que ela está fazendo muitas tarefas diferentes, experimente dividir o código em duas funções. 
...
Evitando que uma função modifique uma lista
    ...
    Em #y utilizamos como argumento unprited_models[:], assim criamos uma cópia da lista (ver propriedade em 2-Listas), e no final foi peidido a lista original para garantir que não foi modificada.
